  ...
  <p class="text-muted text-center">${comMember.code_state }</p>
	...
	<table>
		<tr class="row text-center" style="padding-left:200px;">
			<td class="col-sm-7">
				<select class="form-control" name="code_state" id="stateCode" style="width:300px;margin-right:30px;">
			        <option>회원 상태</option>
			...
			<td class="col-sm-5">
				<button type="button" class="btn btn-block btn-secondary" id="modifyStateCodeBtn" onclick="modifyStateCode();" style="width:180px;">회원 상태 변경</button>
			</td>
		</tr>
	</table>
	<br/>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <table class="table table-bordered">
			...
			<tr>
				<th>사업자 등록증</th>
				<td><button class="col-sm-5 btn btn-block" type="button" onclick="self.location='receiveDoc?fileName=${comMember.com_bsns_reg_license}&id=${comMember.mem_email }';" style="margin-left:150px;"><i class="fas fa-folder-open fa-3x"></i>다운로드</button></td>
			</tr>
			<tr>
				<th>소상공인 증명서</th>
				<td><button class="col-sm-5 btn btn-block" type="button" onclick="self.location='receiveDoc?fileName=${comMember.com_bsns_cert}&id=${comMember.mem_email }';" style="margin-left:150px;"><i class="fas fa-folder-open fa-3x"></i>다운로드</button></td>
			</tr>
		 </table>
      </div>
      <!-- /.card-body -->
          	...
          	<c:if test="${!empty comMemberProjectPayList }">
	          	<c:forEach items="${comMemberProjectPayList}" var="comMemberProjectPayDetail">
		            <tr>
		              ...
		              <td style="text-align:left;max-width: 400px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;"><img src="<%= request.getContextPath() %>/project/picture/get?picture=${comMemberProjectPayDetail.pjt_atc_sum_name}" onerror="this.src='https://source.unsplash.com/category/nature'" alt="image" style="width:100px;height:100px;">&nbsp;${comMemberProjectPayDetail.pjt_title }</td>
          	...
          	<c:if test="${!empty comMemberProjectList }">
	          	<c:forEach items="${comMemberProjectList}" var="comMemberProject">
		            <tr>
		              ...
		              <td style="text-align:left;max-width: 400px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;"><img src="<%= request.getContextPath() %>/project/picture/get?picture=${comMemberProject.pjt_atc_sum_name}" style="width:100px;height:100px;">&nbsp;${comMemberProject.pjt_title }</td>
