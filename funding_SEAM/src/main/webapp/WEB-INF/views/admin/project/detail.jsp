      	...
      	<table class="table table-bordered">
      		<c:forEach items="${projectInProgressList }" var="projectInProgress">
      			<c:if test="${project.pjt_num eq projectInProgress.pjt_num }">
      				<tr>
      				<th rowspan="2"><h3 style="padding-top:32px;">진행중인 프로젝트입니다!</h3></th>
	      				<td>
	      				...
	      				<tr>
     						<c:if test="${project.pjt_cash <= project.pjtPaySum }">
     							<c:if test="${project.pjt_calculate_code eq 1 }">
		     						<th>
			     						<h3>목표 금액에 달성하여 정산 대기중인 프로젝트입니다!</h3>
	    	...
	    	<c:forEach items="${standByToCalcList }" var="standByToRefund">
					...
					<c:if test="${project.pjt_type_code eq 2 }">
						<c:if test="${project.pjt_cash <= project.pjtPaySum }">
							<c:if test="${empty project.pjt_repaydate }">
								<tr>
									<th style="width:63%;">
				 						<h3>대출 상환 대기중인 프로젝트입니다!</h3>
				 						<h4 id="repayDate" style="color:red;display:none;font-size:18px;"></h4>
				 					</th>
				  					<td>
					  					<select class="form-control" id="forRepaySelect" style="width:200px;float:left;margin-left:15px;">
						  					<option>--상환 여부--</option>
						  					<c:if test="${project.pjt_calculate_code eq 1}">
      		...
      		<c:if test="${project.pjt_state_code eq 1 }">
				<tr>
					<th style="width:65%;"><h3>프로젝트 등록 승인 대기중인 프로젝트입니다!</h3></th>
					<td>
						<select class="form-control" id="stateSelect" style="width:200px;float:left;">
					  		<option value="2">승인</option>
					  		<option value="3">보류</option>
					  	</select>
					  	<button type="button" class="btn btn-secondary" id="saveStateBtn" style="width:100px;" onclick="saveState();">저장</button>
					</td>
				</tr>
     		</c:if>	
